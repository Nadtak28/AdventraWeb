    ...
    (node) => {
      ...
      observerRef.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && hasNextPage) {
          ...
          dispatch(LoadMoreCitiesService(currentPage + 1));
        }
      });

      if (node) observerRef.current.observe(node);
    },
    [loading, loadingMore, hasNextPage, currentPage, dispatch]
  );

  if (loading && cities.length === 0)
    return (
      <div className="flex items-center justify-center py-20 dark:bg-[#1a1f2e] bg-white min-h-screen">
    ...
    <div className="min-h-screen dark:bg-[#1a1f2e] bg-gradient-to-br bg-white">
      ...
      <div className="px-4 sm:px-6 md:px-10 lg:px-20 py-8">
        ...
        <div className="text-center mb-12 animate-fade-in">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold dark:from-white dark:to-[#519489] bg-gradient-to-r from-[#519489] bg-clip-text text-transparent mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {(cities || []).map((city, index) => {
            const isLastCity = cities.length === index + 1;

            return (
              <div
                key={city.id}
                ref={isLastCity ? lastCityRef : null}
                onClick={() => navigate(`/cities/${city.id}`)}
                className="group relative from-white bg-gradient-to-br dark:from-gray-800 dark:to-gray-900 rounded-3xl overflow-hidden shadow-lg card-hover-effect cursor-pointer glow-effect animate-slide-up"
              ...
              >
                ...
                <div className="relative h-48 sm:h-52 md:h-56 w-full overflow-hidden">
                  {city.images && city.images.length > 0 ? (
                    <>
                      ...
                      {/* City Name Overlay */}
                      <div className="absolute bottom-4 dark:text-amber-300 left-4 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                ...
                <div className="p-6 relative">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-[#519489] group-hover:to-[#abf1e5] group-hover:bg-clip-text transition-all duration-300">
                      {city.name}
                    </h3>
                  ...
                  <p className="text-gray-600 dark:text-white text-sm leading-relaxed line-clamp-3 mb-6 group-hover:text-gray-700 dark:group-hover:text-gray-200 transition-colors duration-300">
                    {city.description}
                  </p>
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100 group-hover:border-purple-100 transition-colors duration-300">
                      <div className="flex items-center space-x-2">
                        <span className="text-xs font-semibold dark:text-[#519489] text-[#519489] bg-gradient-to-r from-blue-50 to-purple-50 dark:bg-gradient-to-br dark:from-gray-800 dark:to-gray-900 px-3 py-1.5 rounded-full border border-blue-100 group-hover:border-purple-200 transition-all duration-300 group-hover:shadow-sm">
                          Explore City
                        </span>
                      </div>
        ...
        {loadingMore && (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#519489] mx-auto mb-3"></div>
              <p className="text-sm font-medium text-[#519489] animate-pulse-slow">
                Loading more cities...
              </p>
            </div>
          </div>
        ...
        {!hasNextPage && cities.length > 0 && (
          <div className="text-center py-12">
            <div className="w-16 h-16 bg-gradient-to-r from-[#519489] to-[#abf1e5] rounded-full flex items-center justify-center mx-auto mb-4 opacity-75">
              <svg
            ...
            <p className="text-gray-500 font-medium">
              You've explored all available cities!
            </p>
        ...
        {cities && cities.length === 0 && !loading && (
