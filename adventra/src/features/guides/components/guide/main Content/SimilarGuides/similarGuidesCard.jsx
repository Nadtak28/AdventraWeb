...
const SimilarGuideCard = ({ guide, index }) => {
  ...
  return (
    <div
      className={`
          bg-gradient-to-br from-white dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 
    ...
    >
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className="text-xl font-bold dark:text-white text-gray-900 mb-2 group-hover:text-[#519489] transition-colors duration-300">
          {guide.name}
        </h3>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => {
              const fullStars = Math.floor(guide.rate); // number of full stars
              const hasHalfStar = guide.rate % 1 >= 0.5; // check for half star
              ...
              if (i < fullStars) {
                ...
                return (
                  <div key={i} className="relative w-4 h-4">
                    ...
                    <svg
                      className="absolute w-4 h-4 text-yellow-400 overflow-hidden"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                      style={{ width: "50%" }}
                    >
            ...
            <span className="ml-2 text-sm dark:text-[#519489] text-gray-600">
              ({guide.reviews_count})
            </span>
          </div>
